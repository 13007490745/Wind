...
void rt_init_thread_entry(void* parameter)
{
	...
	}
	{
		...
		rt_thread_idle_sethook(SystemTickHook);	
	...
	}
	{
		...
		TmpSampleProcessInit();//30->23
		FwsCH2OdectProcessInit();//22->21
		FwsCOdectProcessInit();//20
		FwsPwrDectProcessInit();//30->24
		FwsGSMProcessInit();//22
		LCDPANELProcessInit();//20->10
		FwsRelayCtlProcessInit();//30
		AudioServer_init();//28
	}
}
